...
st.markdown("""
    ...
    .result-popup {
        ...
        /* 要望：背景を水色に (TailWind CSS sky-100) */
...
# ==========================================
    ...
    # ここからデザイン修正：等級による色分けをやめて、要望通り背景を水色、文字を黒太字に
    title_text = "🎉 大当たり！ 🎉" if rank == 1 else "✨ 当たり！ ✨" if rank == 2 else "ガチャ結果"
    
    # 既存の動的な色指定を削除し、固定のCSSクラス `result-popup` を使用
    st.markdown(f"""
...
with st.expander("⚙️ 管理者設定"):
    ...
    edited_winner_df = st.data_editor(df_winners, disabled=["日時", "お名前", "景品名", "等級", "使用日時"], hide_index=True, use_container_width=True, key="winner_editor")
    if st.button("チェック状態を保存", key="save_winners_btn"):
        try:
            # スプレッドシートのwinnersタブを更新
            conn.update(worksheet="winners", data=edited_winner_df)
            st.success("スプレッドシートの当選者データを更新しました！")
            time.sleep(1); st.rerun()
        except Exception as e:
            st.error(f"当選者データ保存エラー: {e}")